...
public class SettlementUploadCTTest  extends FirstLifeTestCaseHelper {
	...
	{
		int i=1;
		username=propertyReader.getCellData(9,i, 19);
		password=propertyReader.getCellData(9, i, 20);
		policyno=propertyReader.getCellData(9, i, 27);
		branchcode=propertyReader.getCellData(9, i, 21);
		path=propertyReader.readApplicationFile("Upload_Path");
		Batchid=propertyReader.readTestData("BatchId");

         // log in for diff user
		 repoterLog = extent.createTest("Settlement Upload CTT Result");
